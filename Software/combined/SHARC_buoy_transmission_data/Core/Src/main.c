...
Code is also provided to send data from the STM32 to other devices using HAL's UART protocol. 
You will need a serial port reader on your PC to receive the data.

The outputed data is compressed-encrypted data.
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if (run ==0) { // so that this only runs once.
		  //start = HAL_GetTick(); // for timing
		  encrypt(inputArray);
		  //end = HAL_GetTick();
		  //t = end-start;
		  ...
		  //HAL_UART_Transmit(&huart2, time_buf, sizeof(time_buf), 1000);
		  run++;
	  }
  }
}
