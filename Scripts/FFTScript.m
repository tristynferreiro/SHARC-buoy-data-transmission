...
filename = '~/Desktop/decompressed.csv';
...
figure(1)
plot(Fv, lowpass(abs(FAcc(Iv))*2, 0.1))
grid
...
figure(2)
plot(Fv, lowpass(abs(FGyro(Iv))*2, 0.1))
grid
...
figure(3)
plot(Fv, lowpass(abs(FTemp(Iv))*2, 0.1))
grid
...
figure(4)
plot(Fv, lowpass(abs(FAll(Iv))*2,0.1))
grid
